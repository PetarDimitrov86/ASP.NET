namespace CameraBazaar.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (user == null)
            {
                ModelState.AddModelError("", "No such email in database.");
                return View(model);
            }
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ApplicationUser currentUser = context.Users.First(x => x.Email == model.Email);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    this.UserManager.AddToRole(user.Id, "Seller");
                    this.context.Sellers.Add(new Seller { UserId = user.Id });
                    context.SaveChanges();
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        // This acts like a regular registration
                        this.UserManager.AddToRole(user.Id, "Seller");
                        this.context.Sellers.Add(new Seller { UserId = user.Id });
                        context.SaveChanges();
