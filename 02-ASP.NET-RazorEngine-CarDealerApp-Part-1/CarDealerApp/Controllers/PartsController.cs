namespace CarDealerApp.Controllers
{
    ...
    public class PartsController : Controller
    {
        ...
        {
            var supplierVms = service.GetPartSuppliers();
            return View(supplierVms);
        }

        [HttpPost]
        [Route("Add")]
        public ActionResult Add([Bind(Include = "Supplier,Name,Price,Quantity")] NewPartBindingModel npbm)
        {
            Part part = service.AddNewPart(npbm);
            part.Supplier = db.Suppliers.Find(npbm.Supplier);
